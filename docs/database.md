...
- `contact_point` — ที่อยู่/เบอร์/เวลาทำการ/ลิงก์แผนที่ ขึ้นกับคู่ (หน่วยงาน, อปท.)
  ไม่ได้ขึ้นกับหน่วยงานอย่างเดียว จึงแยกจาก `issuing_agency`
  (`map_url` มีทั้งที่ `local_authority` และ `contact_point` ด้วยเหตุผลเดียวกับที่อยู่:
  กองช่างอาจไม่ได้อยู่อาคารเดียวกับสำนักงานใหญ่ของ อปท. ตำแหน่งจึงเป็นคุณสมบัติ
  ของ*สำนักงานนั้น* ค่าว่างที่จุดติดต่อแปลว่า "ใช้หมุดของ อปท." ไม่ใช่ "ไม่มีหมุด")
